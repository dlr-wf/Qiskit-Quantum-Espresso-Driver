...
__global__ void sum_p_reduction_iijj(const thrust::complex<double>* c_ip, const double* p_vec, thrust::complex<double>* output,
...
bool write_output(thrust::host_vector<thrust::complex<float>> output, std::string filename) {
    ...
    for (int idx=0; idx<output.size(); ++idx) {
        ...
        outFile << i << " " << i << " " << j << " " << j << " " << output[idx].real() << " " << output[idx].imag() << "\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Calculating frozen core mean-field energy component g_iijj" << std::endl;
    ...
    sum_p_reduction_iijj<<<blocks, ThreadsPerBlock>>>(thrust::raw_pointer_cast(dev_c_ip.data()), // Takes ~4min for one band
    ...
    std::string filename_output {base_folder+"eri_frozen_iijj_"+std::to_string(start_band)+"_"+std::to_string(end_band)+".txt"};
