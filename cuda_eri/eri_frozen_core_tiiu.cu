...
__global__ void sum_p_reduction_tiiu(const thrust::complex<double>* c_ip, const double* p_vec, thrust::complex<double>* output,
                                     const int c_ip_size, const int p_size, const int n_bands_active, const int n_bands_core, const int n_waves) {
    // Calculates h_tiiu where t,u are active space indices and i is a core index
    ...
    if (blockId < n_bands_active * n_bands_active * n_bands_core) {
        ...
        for (int p_index = threadId; p_index < n_waves; p_index += blockDim.x) {
            ...
            for (int q_index = 0; q_index < n_waves; ++q_index) {
                ...
                thrust::complex<double> c_jq_conj = thrust::conj(c_ip[i*n_waves+q_index]);
...
bool write_output(thrust::host_vector<thrust::complex<float>> output, std::string filename,
    ...
    for (int idx=0; idx<output.size(); ++idx) {
        ...
        outFile << t << " " << i << " " << i << " " << u << " " << output[idx].real() << " " << output[idx].imag() << "\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Calculating frozen core effective potential component g_tiiu" << std::endl;
    ...
    sum_p_reduction_tiiu<<<blocks, ThreadsPerBlock>>>(thrust::raw_pointer_cast(dev_c_ip.data()), // Takes ~4min for one band
    ...
    std::string filename_output {base_folder+"eri_frozen_tiiu_active_"+std::to_string(start_band_active)+"_"+std::to_string(end_band_active)
