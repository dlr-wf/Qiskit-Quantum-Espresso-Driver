...
std::vector<std::vector<int>> calc_num_independent(int n) {
    std::vector<std::vector<std::vector<std::vector<bool>>>> V(n, std::vector<std::vector<std::vector<bool>>>(n, std::vector<std::vector<bool>>(n, std::vector<bool>(n, false))));
    std::vector<std::vector<int>> independent_indices;
        ...
        for (int k = 0; k < n; k++) {
            for (int j = 0; j < n; j++) {
                for (int i = 0; i < n; i++) {
                    ...
                    V[i][j][k][l] = true;
                    V[j][i][l][k] = true; // V_ijkl = V _jilk (swap)
                    V[l][k][j][i] = true; // V_ijkl = V*_lkji (hermiticity)
                    V[k][l][i][j] = true; // V_ijkl = V*_klij (swap+hermiticity)

                    independent_indices.push_back({i, j, k, l});
                }
            }
        }
    }
...
__global__ void sum_p_reduction(const thrust::complex<double>* c_ip, const double* p_vec, thrust::complex<double>* output,
                                const int c_ip_size, const int p_size, const int n_bands, const int n_waves,
                                const int* indices, const int n_indices) {
    // Shapes:
    ...
    if (blockId < n_indices/4) {
        int i = indices[blockId*4+0];
        int j = indices[blockId*4+1];
        int k = indices[blockId*4+2];
        int l = indices[blockId*4+3];
                    ...
                    for (int r_index = 0; r_index < n_waves; ++r_index) {
                        ...
                        if (p_minus_r_equals_s_minus_q) {
                            partialSum[threadId] += c_ip_conj
                                * c_jq_conj
                                * c_ip[k*n_waves+r_index]
                                * c_ip[l*n_waves+s_index]
...
bool write_output(thrust::host_vector<thrust::complex<float>> output, std::string filename) {
    ...
    int n_bands = int(std::pow(numComplexElements, 0.25));
    ...
    for (int idx=0; idx<output.size(); ++idx) {
        ...
        int k = (idx / (n_bands * n_bands)) % n_bands;
        int l = (idx / (n_bands * n_bands * n_bands)) % n_bands;
        outFile << i << " " << j << " " << k << " " << l << " " << output[idx].real() << " " << output[idx].imag() << "\n";
    }
...
bool write_output_sym(thrust::host_vector<thrust::complex<float>> output,
                      std::vector<std::vector<int>> independent_indices,
                      int n_bands, std::string filename) {
    // This function produces duplicates in the output! If this is a problem use write_output instead .

    // Open the file for writing
    std::ofstream outFile(filename);

    if (output.size() != independent_indices.size()) {
        std::cerr << "output size (" << output.size() << ") does not equal number of independent indices (" <<
        independent_indices.size() << ")" << std::endl;
        return false;
    }
    ...
    // Write the number of complex elements to the file
    outFile << n_bands*n_bands*n_bands*n_bands << " " << n_bands << "\n";

    int i,j,k,l = 0;
    // Write each complex number to the file
    outFile << std::setprecision(std::numeric_limits<float>::max_digits10);
    for (int idx=0; idx<independent_indices.size(); ++idx) {
        i = independent_indices[idx][0];
        j = independent_indices[idx][1];
        k = independent_indices[idx][2];
        l = independent_indices[idx][3];

        // Independent element
        outFile << i << " "  << j << " "  << k << " "  << l << " "  <<
                output[idx].real() << " " << output[idx].imag() << "\n";
        // swap symmetry
        if (i != j || k != l) {
            outFile << j << " "  << i << " "  << l << " "  << k << " "  <<
                    output[idx].real() << " " << output[idx].imag() << "\n";
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        std::cerr << "Number of arguments needs to be two, start band and end band, but is " << argc-1 << "!" << std::endl;
        return 1;
    }

    // Note: Using double instead of float yields enough precision so that the electron repulsion integrals have
    //       all needed symmetries up to numpy.allclose precision.
    //       Using float does not yield that precision but allows for more threads per block
    //       Maybe use floats and enforce neceassary symmetries manually by only calculating
    //       independent matrix elements and apply the symmetries to get the full matrix without
    //       further calculations of matrix elements
    std::cout << "Calculating independent electron repulsion integrals h_ijkl" << std::endl;
    ...
    if (end_band < 0 || end_band > n_bands_all || end_band <= start_band) {
        std::cerr << "End band (" << end_band << ") is invalid! Needs to be 0 or larger and must not be larger than "
                  << n_bands_all << " and cannot be equal or smaller than start band (" << start_band << ")!" << std::endl;
        return 1;
    }


    std::vector<std::vector<int>> independent_indices = calc_num_independent(n_bands);

    // Flatten the input vector into a single array
    std::vector<int> independent_indices_flattened;
    for (const auto& inner_vector : independent_indices)
    {
        independent_indices_flattened.insert(independent_indices_flattened.end(),
                                             inner_vector.begin(), inner_vector.end());
    }
    thrust::device_vector<int> independent_indices_device = independent_indices_flattened;
    ...
    int output_size = independent_indices.size();
    ...
    sum_p_reduction<<<blocks, ThreadsPerBlock>>>(thrust::raw_pointer_cast(dev_c_ip.data()),
                                   thrust::raw_pointer_cast(dev_p.data()),
                                   thrust::raw_pointer_cast(devOutput.data()), size_c_ip, size_p, n_bands, n_waves,
                                   thrust::raw_pointer_cast(independent_indices_device.data()), independent_indices_device.size());
    ...
    thrust::device_vector<thrust::complex<double>> hostOutput_full(n_bands*n_bands*n_bands*n_bands,
                                                                   thrust::complex<double>(0.0f, 0.0f));
    int i,j,k,l;
    for (int idx=0; idx<independent_indices.size(); ++idx) {
        i = independent_indices[idx][0];
        j = independent_indices[idx][1];
        ...
        int ijkl = i + n_bands*j + n_bands*n_bands*k + n_bands*n_bands*n_bands*l;
        int jilk = j + n_bands*i + n_bands*n_bands*l + n_bands*n_bands*n_bands*k;
        int lkji = l + n_bands*k + n_bands*n_bands*j + n_bands*n_bands*n_bands*i;
        int klij = k + n_bands*l + n_bands*n_bands*i + n_bands*n_bands*n_bands*j;

        hostOutput_full[ijkl] = hostOutput[idx]; // Independent element
        hostOutput_full[jilk] = hostOutput[idx]; // Swap symmetry
        hostOutput_full[lkji] = hostOutput[idx]; // Hermiticity symmetry
        hostOutput_full[klij] = hostOutput[idx]; // Swap+hermiticity symmetry
    }

    std::string filename_output {base_folder+"eri_sym_cu"+std::to_string(start_band)+"_"+std::to_string(end_band)+".txt"};
    bool writeOutput = write_output(hostOutput_full, filename_output);
    if (writeOutput) {
