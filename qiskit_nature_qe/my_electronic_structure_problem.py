...
from qiskit_nature.second_q.problems.electronic_properties_container import (
...
from qiskit_nature.second_q.problems.electronic_basis import ElectronicBasis
...
class ElectronicStructureProblem(BaseProblem):
    ...
    def interpret(
        ...
        result.computed_energies = np.asarray(
            [e.real for e in eigenstate_result.eigenvalues]
        )
    ...
    def get_default_filter_criterion(
        ...
        def filter_criterion(self, eigenstate, eigenvalue, aux_values):
            verbose = False
            print(eigenvalue)
            if np.abs(eigenvalue - (-1.1794533970297665)) < 1e-2:
                verbose = True
            ...
            num_particles_close = np.isclose(
                eval_num_particles[0], self.num_alpha + self.num_beta
            )
            ...
            if verbose is True:
                print(f"aux_values: {aux_values}")
                print(f"eval_num_particles: {eval_num_particles}")
                print(f"eval_num_particles[0]: {eval_num_particles[0]}")
                print(
                    f"self.num_alpha + self.num_beta: {self.num_alpha + self.num_beta}"
                )
                print(f"eval_angular_momentum: {eval_angular_momentum}")
                print(f"eval_angular_momentum[0]: {eval_angular_momentum[0]}")
                print(f"0.0: {0.0}")
